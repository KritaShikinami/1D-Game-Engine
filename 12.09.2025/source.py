...
CHARACTER_TYPES = {
    ...
    "TESTSUBJECT01-DEATH": {
        "speed": 0,
        "size": 32,
        "walk_sequence": ["Assets/Arts/default_character_death_head_1.png",
                          "Assets/Arts/default_character_death_head_2.png",
                          "Assets/Arts/default_character_death_head_3.png",
                          "Assets/Arts/default_character_death_head_4.png",
                          "Assets/Arts/default_character_death_head_5.png"],
        "frame_delay": 10,
        "head_hp": 0,
        "body_hp": 0,
        "leg_hp": 0,
    ...
    "TESTSUBJECT01-GUN": {
        "speed": 1,
        "size": 32,
        "walk_sequence": ["Assets/Arts/default_character_with_gun_sprite_1.png",
                          "Assets/Arts/default_character_with_gun_sprite_2.png",
                          "Assets/Arts/default_character_with_gun_sprite_3.png",
                          "Assets/Arts/default_character_with_gun_sprite_2.png"],
        "frame_delay": 15,
        "head_hp": 100,
        "body_hp": 100,
    ...
    "TESTSUBJECT01-FIRING": {
        "speed": 1,
        "size": 32,
        "walk_sequence": ["Assets/Arts/default_character_firing_gun_sprite_1.png",
                          "Assets/Arts/default_character_firing_gun_sprite_2.png",
                          "Assets/Arts/default_character_firing_gun_sprite_3.png",
                          "Assets/Arts/default_character_firing_gun_sprite_4.png",
                          "Assets/Arts/default_character_firing_gun_sprite_5.png"],
        "frame_delay": 5,
        "head_hp": 100,
        "body_hp": 100,
        "leg_hp": 100,
...
def create_npc(npc_type, x, y, character_types, aggressive=False):
    data = character_types[npc_type]
    npc = {
        "type": npc_type,
        "x": x,
        "y": y,
        ...
        "facing_left": False,
        "dead": False,
        "total_hp": data.get("total_hp", 100),
        "move_timer": 0,
        "move_interval": random.randint(30, 90),
        "direction": 0,
        "firing": False,
...
def update_npc(npc, world_width):
    """NPC'nin basit AI ile sağa sola dolaşmasını ve ara sıra durmasını sağlar."""
    if npc["dead"]:
        return  # ölü NPC hareket etmez

    npc["move_timer"] += 1
    if npc["move_timer"] >= npc["move_interval"]:
        npc["move_timer"] = 0
        npc["direction"] = random.choice([-1, 0, 1])

    if npc["direction"] != 0:
        npc["x"] += npc["direction"] * npc["speed"]
        npc["facing_left"] = True if npc["direction"] < 0 else False

        if npc["x"] < 0:
            npc["x"] = 0
            npc["direction"] = 1
            npc["facing_left"] = False
        elif npc["x"] > world_width - npc["size"]:
            npc["x"] = world_width - npc["size"]
            npc["direction"] = -1
        ...
        if npc["frame_timer"] >= npc["frame_delay"]:
            npc["frame_timer"] = 0
            npc["current_frame"] = (npc["current_frame"] + 1) % len(npc["walk_frames"])
    else:
        npc["current_frame"] = 0

def draw_npc(screen, npc, camera_x):
    sprite = pygame.transform.scale(npc["walk_frames"][npc["current_frame"]], (npc["size"], npc["size"]))
    if npc["facing_left"]:
        sprite = pygame.transform.flip(sprite, True, False)
    screen.blit(sprite, (npc["x"] - camera_x, npc["y"]))

def update_npc_passive(npc, world_width):
    if npc["dead"]:
        return
    ...
    if npc["direction"] != 0:
        npc["x"] += npc["direction"] * npc["speed"]
        npc["facing_left"] = npc["direction"] < 0
...
# --- Agresif NPC güncelleme ---
def update_npc_aggressive(npc, player, world_width):
    from source import CHARACTER_TYPES  # karakter tiplerine erişim

    # Eğer ateş etmiyorsa normal hareket etsin
    if not npc.get("firing", False):
        update_npc_passive(npc, world_width)

    # Oyuncuya saldırma kontrolü
    npc["attack_timer"] += 1
    if npc["attack_timer"] >= npc["attack_interval"]:
        npc["attack_timer"] = 0
        if abs(npc["x"] - player["x"]) < 100 and not npc["dead"]:
            # Ateş animasyonuna geç
            npc["firing"] = True
            npc["type"] = "TESTSUBJECT01-FIRING"
            npc["walk_frames"] = [
                pygame.image.load(path).convert_alpha()
                for path in CHARACTER_TYPES["TESTSUBJECT01-FIRING"]["walk_sequence"]
            ]
            npc["frame_delay"] = CHARACTER_TYPES["TESTSUBJECT01-FIRING"]["frame_delay"]
            npc["current_frame"] = 0
            npc["frame_timer"] = 0

            # Oyuncuya hasar ver
            player["total_hp"] -= 10
            print("Oyuncu saldırıya uğradı! HP:", player["total_hp"])

    # Ateş animasyonu bittiyse tekrar silahlı yürüyüşe dön
    if npc.get("firing", False):
        npc["frame_timer"] += 1
        if npc["frame_timer"] >= npc["frame_delay"]:
            npc["frame_timer"] = 0
            npc["current_frame"] += 1
            if npc["current_frame"] >= len(npc["walk_frames"]):
                npc["firing"] = False
...
def is_hit_by_mouse(mouse_pos, entity, camera_x):
    """
    Fare pozisyonunun, kamera ofseti hesaba katılarak
    verilen entity'nin (NPC, oyuncu vb.) dikdörtgen alanına
    denk gelip gelmediğini kontrol eder.
    """
    screen_x = entity["x"] - camera_x
    rect = pygame.Rect(screen_x, entity["y"], entity["size"], entity["size"])
    return rect.collidepoint(mouse_pos)

def get_cursor_type(mouse_pos, entities, camera_x):
    """
    Ekrandaki tüm entity'ler için fare imlecinin saldırı mı
    yoksa normal mi olacağını belirler.
    """
    for entity in entities:
        if not entity["dead"] and is_hit_by_mouse(mouse_pos, entity, camera_x):
            return "attack"
    return "normal"
...
"""
# --- V.A.T.S. için, denemeyin ve kullanmayın. Buglar temizlenmedi.
class BodyPart:
    def __init__(self, name, position, length, angle, health=100):
        self.name = name
    ...
    def get_end(self):
        x = self.position[0] + self.length * math.cos(self.angle)
        y = self.position[1] + self.length * math.sin(self.angle)
        return (x, y)
    ...
    def render(self, surface, draw_line, draw_circle):
        end = self.get_end()
        color = self.get_color()
        draw_line(surface, self.position, end, color, 4)
        draw_circle(surface, end, 6, color)
...
class SkeletonCharacter:
    ...
    def add_part(self, name, offset, length, angle, health=100):
        part_pos = (self.position[0] + offset[0], self.position[1] + offset[1])
        part = BodyPart(name, part_pos, length, angle, health)
        self.parts.append(part)

    def render(self, surface, draw_line, draw_circle):
        for part in self.parts:
            part.render(surface, draw_line, draw_circle)

class StatusPanel:
    def __init__(self, character):
        self.character = character

    def render(self, surface, draw_text, font, position):
        x, y = position
        for i, part in enumerate(self.character.parts):
            label = f"{part.name}: {part.health}%"
            color = part.get_color()
            draw_text(surface, label, (x, y + i * 20), font, color)
"""
# --- Debug Mode ---
